...
// Logo « G » de Google en SVG inline : pas de requête externe, fonctionne hors ligne
const GoogleLogo = () => (
...
export const LoginWithGoogle: React.FC<LoginProps> = ({ isRegistering }) => {
  const { setIsLoading, setNotice } = useUserDataContext();
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // La page part vers Google puis revient : getRedirectResult est géré par le contexte
    } catch (error) {
      setIsLoading(false);
  ...
  return (
    ...
    >
      <GoogleLogo />
      {isRegistering ? "Créer mon compte avec Google" : "Connexion avec Google"}
    </Button>
  );
};
